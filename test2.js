...
function onConnect(status) {
    if (status == Strophe.Status.CONNFAIL) {
        ...
        connection.addHandler(onPres, null, 'presence', null, null, null);
...
function onIQ(iq){
    ...
    var subscription = elems.getAttribute('subscription');
    if(subscription){
        var iq = $iq({
            from:jid,
            id:'b12345',
            type:'set'
        }).c('query',{xmlns:'jabber:iq:roster'}).c('item',{jid:from,name:'lisi',subscription:'both'}).c('group').t('servicerGroup');
        connection.send(iq.tree());
    }
}

function onPres(pres){
    ...
    switch(type){
        case 'subscribe':
            var reply = $pres({type:'subscribed',to:from,from:to});
